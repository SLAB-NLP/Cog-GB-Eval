...
def unite_human_ids(ds_name, humans_covered_sentences):
    humans_ids = set()
    for key in humans_covered_sentences[ds_name]:
        if type(humans_covered_sentences[ds_name][key]) == dict:
            for inner_dict in humans_covered_sentences[ds_name][key]:
                humans_ids.update(humans_covered_sentences[ds_name][key][inner_dict])
        else:
            humans_ids.update(humans_covered_sentences[ds_name][key])
    return humans_ids
...
if __name__ == '__main__':
    ...
    human_ids = get_human_ids_covered()
